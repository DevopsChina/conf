import { ADD_POTENTIAL_PARTNER, ADD_POTENTIAL_PARTNER_FAILURE, ADD_POTENTIAL_PARTNER_SUCCESS, FETCH_PARTNERS, FETCH_PARTNERS_FAILURE, FETCH_PARTNERS_SUCCESS, } from '../constants';
import { db } from '../db';
const _getPartnerItems = (groupId) => db()
    .collection('partners')
    .doc(groupId)
    .collection('items')
    .get()
...
export const partnersActions = {
    ...
    fetchPartners: () => (dispatch) => {
        ...
        db()
            .collection('partners')
            .get()
            .then((snaps) => Promise.all(snaps.docs.map((snap) => Promise.all([snap.data(), snap.id, _getPartnerItems(snap.id)]))))
