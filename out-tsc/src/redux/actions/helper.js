...
export const helperActions = {
    storeUser: (user) => {
        ...
        if (user) {
            ...
            const initialProviderId = (user.providerData && user.providerData[0].providerId) || user.initialProviderId;
        ...
        if (!userToStore.signedIn)
            store.dispatch({ type: WIPE_PREVIOUS_FEEDBACK });
    },
