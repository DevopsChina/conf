...
describe('auth-required', () => {
    ...
    let shadowRoot;
    beforeAll(async () => {
        const render = await fixture(html `
        <auth-required>
    ...
    it('shows unauthenticated prompt', () => {
        expect(shadowRoot.querySelector('mwc-button')).not.toHaveAttribute('hidden');
    ...
    it('shows authenticated content', async () => {
        ...
        expect(shadowRoot.querySelector('mwc-button')).toHaveAttribute('hidden');
        const slots = shadowRoot.querySelectorAll('slot');
        expect(slots).toHaveLength(2);
...
//# sourceMappingURL=auth-required.test.js.map