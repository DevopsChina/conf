...
class SessionDetails extends SpeakersHoC(ReduxMixin(mixinBehaviors([IronOverlayBehavior], PolymerElement))) {
    static get template() {
        return html `
    ...
    _sessionUpdate() {
        ...
        if (day && startTime) {
            ...
            const now = new Date();
            const convertedTimezoneDate = new Date(new Date(`${day} ${startTime}`).getTime() +
