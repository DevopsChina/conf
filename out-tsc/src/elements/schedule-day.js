...
class ScheduleDay extends PolymerElement {
        ...
        if (active && window.location.hash) {
            ...
            if (selectedTime) {
                requestAnimationFrame(() => {
                    const Elements = window.HOVERBOARD.Elements;
    ...
    _showAddSession(timeslot, onlyFeatured) {
        return (onlyFeatured &&
            !timeslot.sessions.reduce((aggregator, sessionBlock) => aggregator + sessionBlock.items.length, 0));
    }
...
//# sourceMappingURL=schedule-day.js.map