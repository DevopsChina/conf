...
class FeaturedVideos extends ReduxMixin(PolymerElement) {
    constructor() {
        super(...arguments);
        this.videos = [];
        this.videosFetching = false;
        this.videosFetchingError = {};
        this.viewport = {};
    ...
    shiftContentRight() {
        ...
        if (currentPosition >= maxRightPosition) {
            const adjustToRight = newX < maxRightPosition || Math.abs(maxRightPosition) - Math.abs(newX) < cardWidth;
            if (adjustToRight) {
    ...
    getVideosDetails() {
        ...
        const videosContainerWidth = parseInt(getComputedStyle(this.$.videoList, null).getPropertyValue('width'));
...
//# sourceMappingURL=featured-videos.js.map